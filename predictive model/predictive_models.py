...
def data_processing():
    ...
    binnames = [0,1]
    #From Histgram, we can observe that 40 is a good point to break down and make the bins
    bins1=[-1,40,100]
    #creat a bin group fot our predicted variable for easier classification
    myData['PopuGroups'] = pd.cut(myData['popularity'], bins1, labels=binnames)
    myData['PopuGroups'] = pd.to_numeric(myData['PopuGroups'], downcast='integer')
    myData.loc[:, myData.columns != 'PopuGroups'] = normalize(myData.loc[:, myData.columns != 'PopuGroups'])
    return myData
...
#Use the full data set to split into train and test data to tran and test the models.
def classifier(models,X,Y):
    test_size = 0.20
    # Set teh threshold value as 0.5, but you can change it to any confident leverl you want.
    threshold = 0.5
    #seed = 7
...
def main():
    ...
    cols = [col for col in myData.columns if col not in ['PopuGroups', 'popularity']]
    # Seperate the train feature data and label data 
    Xdata = myData[cols]
    Ydata = myData.loc[:, myData.columns == 'PopuGroups']
