...
def basicstats(df,columns):
    ...
    description.to_csv(filename, sep = ',',encoding='utf-8-sig', 
                       mode='a', index = False, float_format='%.8f')
    
       
#define a column list with 11 attributes
columnnames=['acousticness','danceability',
 'duration_ms','energy','instrumentalness',
 'liveness','loudness','speechiness',
 'tempo','popularity','Movie_rate']
#print out description results on these 11 columns
...
# popularity >= 70 : very popular; category number: 4
# 45<= popularity <70 : popular; category number: 3
# 20<= popularity <45 : common; category number: 2
# popularity < 20 : not popular; category number: 1

def poplevel (row):
    if row['popularity'] >=70 :
       return 4
    if row['popularity'] <70 and row['popularity']>=45 :
       return 3
    if row['popularity'] <45 and row['popularity']>=20 :
       return 2
...
def main():
 ...
 'Movie_gross','Movie_runtime']
    ...
    #first,use 'liveness','tempo','loudness','energy'
    #to detect outliers
    odcolumns=['tempo','loudness','energy']
    ODdf=myData[odcolumns]
    ODnorm=StandardScaler().fit_transform(ODdf)
    getDistance(ODnorm,odcolumns,'distance1.png')
    #after checking the distance, do DBSCAN
    dbres=ODdbscan(myData,odcolumns,'dbscan1.png',.4,10)
